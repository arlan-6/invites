...
const Page: FC<PageProps> = async ({ params }) => {
	...
	const rsvpTrack: RsvpEntry[] = Array.isArray(invite.rsvpTrack)
		? invite.rsvpTrack
		: [];

	const goingCount = rsvpTrack.filter((r) => r.attendance === "going").length;
	const notGoingCount = rsvpTrack.filter(
		(r) => r.attendance === "notGoing",
	).length;
			...
			<Card className="shadow-lg">
				...
				<CardContent>
					<div className="mb-6 flex flex-wrap items-center gap-4 text-lg font-medium">
						...
						<Badge
							variant="outline"
							className="border-green-500 bg-green-50 px-3 py-1 text-base text-green-700"
						>
						...
						<Badge
							variant="outline"
							className="border-red-500 bg-red-50 px-3 py-1 text-base text-red-700"
						>
						...
						<div className="overflow-hidden rounded-md border">
							<Table>
								<TableHeader className="bg-muted/50">
									<TableRow>
										<TableHead className="w-[40%] px-4 py-3 text-left font-semibold">
											Name
										</TableHead>
										<TableHead className="w-[30%] px-4 py-3 text-left font-semibold">
											Answer Date
										</TableHead>
										<TableHead className="w-[30%] px-4 py-3 text-center font-semibold">
											Attendance
										</TableHead>
									</TableRow>
								</TableHeader>
									...
									{rsvpTrack.map((r, i) => (
										<TableRow key={i} className="hover:bg-muted/30">
											...
											<TableCell className="px-4 py-3 text-center">
												<Badge
													variant={
														r.attendance === "going" ? "default" : "destructive"
													}
													className={cn(
				...
				{rsvpTrack.length > 0 && (
					<TableCaption className="pb-4">
						Detailed list of RSVP responses.
					</TableCaption>
				)}
			</Card>
		</div>
