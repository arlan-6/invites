...
import { updateUserCredits } from "@/lib/userUtils";
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Prevent this user from signing in. Banned users remain in the user
						list.
					</DialogDescription>
				</DialogHeader>
				...
				{/* <DialogFooter> */}
				<DialogClose>
					<Button variant="outline" disabled={isBanning}>
						Cancel
					</Button>
				</DialogClose>
...
export default function UsersTable() {
	...
	const fetchUsers = useCallback(
		async (page: number, size: number, showLoadingIndicator = true) => {
			// Decide whether to show full table loading or action loading
			const setLoading = showLoadingIndicator
				? setIsLoading
				: setIsActionLoading;
			setLoading(true);
			setError(null);
			...
			try {
				const response = await authClient.admin.listUsers({
					query: {
						limit: size,
						offset: (page - 1) * size, // You might add sortBy here if needed
				...
				if (data) {
					...
					console.warn(
						"Received unexpected response structure or no data:",
						response,
					);
					setUsers([]);
					setTotalUsers(0);
					setError(
						new Error("Failed to fetch users: Invalid response format."),
					);
				}
			} catch (err) {
    ...
    const handleCreditChange = async (userId:string,creditValue:number)=>{
        setIsActionLoading(true);
        try {
           const res =  await updateUserCredits(userId,creditValue)
           if(res.error){
            alert(`Error setting credit`);
           }
			refreshCurrentUserList(); // Refresh data
            ...
            console.error("Failed to set credit:", err);
			alert(`Error setting credit: ${err.message}`); // Replace with toast
			setIsActionLoading(false); // Stop loading on error
        }
    }
	...
	if (error && users.length === 0) {
		// Show error prominently if no users could be loaded
		return (
		...
		<div className="space-y-4">
			{error && !isLoading && (
				<div className="p-2 text-center text-yellow-600 bg-yellow-100 border border-yellow-300 rounded">
					Non-critical Error: {error.message} (Displaying possibly stale data)
				</div>
			)}
										...
										<TableCell>
											<span
												className={cn(
													...
													user.credits > 0 && "text-amber-400",
													user.credits > 3 && "text-green-400",
												)}
											>
												{user.credits}
											</span>
													...
													<DropdownMenuSub>
														...
														<DropdownMenuPortal>
															<DropdownMenuSubContent>
																...
																<DropdownMenuItem onSelect={()=>handleCreditChange(user.id,user.credits +1)}>+1</DropdownMenuItem>
																<DropdownMenuItem onSelect={()=>handleCreditChange(user.id,user.credits +5)}>+5</DropdownMenuItem>
																<DropdownMenuItem onSelect={()=>handleCreditChange(user.id,user.credits -5)}>-5</DropdownMenuItem>
																<DropdownMenuItem onSelect={()=>handleCreditChange(user.id,user.credits -1)}>-1</DropdownMenuItem>
															</DropdownMenuSubContent>
														</DropdownMenuPortal>
						...
						{totalPages > 1 && (
							<div className="flex items-center space-x-2">
								...
								<Input
									...
									onBlur={() => {
										// Reset if blurred with invalid input
										const pageNum = parseInt(pageInput, 10);
						...
						{totalPages > 1 && (
							<div className="flex items-center space-x-1">
								...
								<Button
									...
									onClick={() => goToPage(currentPage + 1)}
									disabled={
										currentPage >= totalPages || isLoading || isActionLoading
									}
